...
| `search_hairpin_junction_routes` | Typed exact-precursor and basal-route results plus route node/hit budgets | Exact released-state projection, one continuous active/surviving strand, content-addressed routes in neutral upstream order, distinct end-agent identities, and separate upstream/local truncation evidence |
