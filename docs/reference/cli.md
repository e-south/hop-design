...
computes exact cardinality without enumerating members, and writes nothing. A
valid specification above its declared `max_members` is reported as `blocked`
with exit code zero. Invalid input returns nonzero.
...
never publishes an authoritative partial set.
