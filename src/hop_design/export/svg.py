...
def render_workflow_svg(view: WorkflowView) -> bytes:
    ...
    width = max(960, sequence_x + longest_track * _BASE_WIDTH + 30)
    ...
    for panel, panel_height in zip(view.panels, panel_heights, strict=True):
        ...
        for index, track in enumerate(panel.tracks):
            ...
            lines.append(
                f'<text x="{sequence_x}" y="{y}" letter-spacing="3">{escape(track.sequence)}</text>'
            )
            lines.append("</g>")
        for feature in panel.features:
            x = sequence_x + feature.span.start.offset * _BASE_WIDTH
        ...
        for pairing in panel.pairings:
            x1 = sequence_x + pairing.left_index * _BASE_WIDTH + 4
            x2 = sequence_x + pairing.right_index * _BASE_WIDTH + 4
