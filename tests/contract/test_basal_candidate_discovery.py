...
def _request(
    *,
    ...
    return hop.BasalCandidateSearchRequest(
...
def test_basal_candidate_search_matches_sanitized_predecessor_set() -> None:
    ...
    result = hop.search_basal_candidates(
        _request(
            left_arm_template=fixture["left_arm_template"],
            right_arm_template=fixture["right_arm_template"],
        ),
        limits=hop.BasalCandidateSearchLimits(max_search_nodes=16, max_hits=16),
    )
    ...
    assert [candidate.rank for candidate in result.hits] == list(range(1, 7))
    assert all(
...
def test_basal_candidate_search_reports_node_and_hit_truncation_separately() -> None:
    node_limited = hop.search_basal_candidates(
        _request(),
        limits=hop.BasalCandidateSearchLimits(max_search_nodes=5, max_hits=16),
    )
    hit_limited = hop.search_basal_candidates(
        _request(),
        limits=hop.BasalCandidateSearchLimits(max_search_nodes=16, max_hits=2),
    )
...
def test_basal_candidate_contracts_reject_projection_and_accounting_drift() -> None:
    ...
    with pytest.raises(ValidationError, match="pairing must equal"):
        hop.BasalCandidate.model_validate_json(json.dumps(candidate_data))
    ...
    for rank, candidate in enumerate(result_data["hits"], start=1):
        candidate["rank"] = rank
    with pytest.raises(ValidationError, match="canonical physical order"):
        hop.BasalCandidateSearchResult.model_validate_json(json.dumps(result_data))
...
def test_basal_candidate_request_is_strict_and_normalizes_iupac_domains() -> None:
    ...
    with pytest.raises(ValidationError):
        hop.BasalCandidateSearchRequest.model_validate(
            {
...
def test_basal_candidate_result_rejects_cardinality_and_domain_drift() -> None:
    ...
    with pytest.raises(ValidationError, match="candidate_space_size"):
        hop.BasalCandidateSearchResult.model_validate_json(json.dumps(cardinality_data))
    ...
    with pytest.raises(ValidationError, match="outside its caller-authored domain"):
        hop.BasalCandidateSearchResult.model_validate_json(json.dumps(domain_data))
...
def test_basal_candidate_result_rejects_unforced_early_stops() -> None:
    ...
    with pytest.raises(ValidationError, match="exhaust the available node budget"):
        hop.BasalCandidateSearchResult.model_validate_json(json.dumps(node_data))
    ...
    with pytest.raises(ValidationError, match="exhaust the available hit budget"):
        hop.BasalCandidateSearchResult.model_validate_json(json.dumps(hit_data))
...
def test_basal_candidate_result_rejects_duplicate_exclusion_summaries() -> None:
    ...
    with pytest.raises(ValidationError, match="must be unique"):
        hop.BasalCandidateSearchResult.model_validate_json(json.dumps(data))


def test_basal_candidate_contract_rejects_wobble_interpretation_drift() -> None:
    result = hop.search_basal_candidates(
    ...
    with pytest.raises(ValidationError, match="honor allow_gt_wobble"):
        hop.BasalCandidate.model_validate_json(json.dumps(data))
...
def test_basal_exclusion_summary_rejects_impossible_status_reason_pair() -> None:
    with pytest.raises(ValidationError, match="status and reason"):
        hop.BasalCandidateExclusionSummary.model_validate_json(
            json.dumps(
                {
                    "status": "reserve",
