...
def _spec() -> hop.HairpinMethodMaterialsSpec:
    return hop.HairpinMethodMaterialsSpec(
        schema="hop.hairpin-method-materials/v1",
        method_id="sanitized-reference",
...
def test_method_materials_resolve_terminal_primer_binding_and_chemistry() -> None:
    plan = hop.resolve_hairpin_method_materials(_spec())

    assert plan.schema_id == "hop.hairpin-method-materials-plan/v1"
...
def test_method_materials_reject_a_primer_that_does_not_bind_its_terminal_handle() -> None:
    ...
    with pytest.raises(
        ...
        hop.resolve_hairpin_method_materials(spec)
...
def test_pre_phosphorylated_route_requires_both_ligation_end_modifications() -> None:
    ...
    with pytest.raises(ValidationError, match="source PCR reverse primer"):
        hop.HairpinMethodMaterialsSpec.model_validate(data)
...
def test_kinase_step_allows_unmodified_ligation_end_oligos() -> None:
    ...
    plan = hop.resolve_hairpin_method_materials(hop.HairpinMethodMaterialsSpec.model_validate(data))

    assert plan.ligation_end_preparation == "kinase_step"


def test_method_materials_plan_rejects_serialized_binding_drift() -> None:
    data = hop.resolve_hairpin_method_materials(_spec()).model_dump(mode="json", by_alias=True)
    data["bindings"][0]["template_span"]["end"]["offset"] = 5

    with pytest.raises(ValidationError, match="binding derivations"):
        hop.HairpinMethodMaterialsPlan.model_validate_json(json.dumps(data))
