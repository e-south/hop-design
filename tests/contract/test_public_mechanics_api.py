...
def test_public_facade_exposes_standalone_mechanics_without_internal_imports() -> None:
    ...
    assert callable(hop.search_released_foldback_geometries)
    assert callable(hop.search_released_foldback_precursors)
    assert callable(hop.search_hairpin_junction_routes)
...
def test_public_facade_manifest_contains_unique_resolvable_names() -> None:
    assert len(hop.__all__) == len(set(hop.__all__))
